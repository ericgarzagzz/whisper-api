...
def create_transcription_task(task_id: uuid.UUID, name: str, file_path: str, status: str):
    with Session(engine) as session:
        task = TranscriptionTask(id=task_id, name=name, file_path=file_path, status=status)
        session.add(task)
        session.commit()
